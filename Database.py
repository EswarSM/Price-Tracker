...
# Main functions add user, add product,
# get details of product, delete a product
def add_user(name, email, phonenumber):
    ...
    mydb = mysql.connector.connect(
        ...
        password="krish2222na", database="krishdb"
    )
...
def add_url(email, title, url, duration, price):
    """
    Takes in email,title,url,duration and price margin and
    adds the to product table
    """
    mydb = mysql.connector.connect(
...
def add_track(email_id, time_stamp, url, price, title, interval, filter_price):
    ...
    try:
        ...
        add_command = (
            "insert into tracker (time, productID, price)  values(%s, %s, %s);"
        )
...
def user_tracker_details(email):
    ...
    try:
        ...
        for ids in list_of_emailids:
            tracker_detail_command = (
                "select time,price from tracker where productID = %s"
            )
            data = (ids,)
        ...
        for position in range(len(list_of_product_details)):
            user_product_details[list_of_product_details[position]] = user_details_list[
                position
            ]
