...
def add_user():
    ...
    status = Database.add_user(name, email, phonenumber)
    print(status)
...
def add_to_fav():
    ...
    status = Database.add_url(email, title, url, interval, price)
...
def delete_url():
    ...
    status = Database.delete_product(email, title, url, interval, price)
    status = str(status)
...
def track_urls():
    ...
    dict = Database.user_tracker_details(email)
    return jsonify(dict)
...
def hour_cycle():
    ...
    while True:
        time.sleep(30)
        try:
            list_of_emailids, list_of_urls, list_of_prices = Database.get_url("0")
            if list_of_urls:
...
def day_cycle():
    ...
    while True:
        time.sleep(45)
        try:
            list_of_emailids, list_of_urls, list_of_prices = Database.get_url("1")
            if list_of_urls:
    ...
    while True:
        ...
        try:
            (
                ...
                list_of_prices,
                list_of_durations,
            ) = Database.get_notification_details()
...
def save_data(emaid_id, url, filter_price, interval):
    ...
    status = Database.add_track(emaid_id, time_stamp, url, price, title, interval, filter_price)
    #print(status)

    price_margin = float(filter_price)
    price = float(price)
    if price_margin >= price:
        status = Database.add_notification(emaid_id, title, url,
                                      filter_price, interval)
        # print(status)
        # return status
